...
impl AnyToken {
    ...
    pub fn new(
        ...
        this.token
            .internal_deposit(router_id.as_ref(), total_supply.into());
        this
    }
    ...
    fn internal_insert_minter(&mut self, account_id: &AccountId) {
        assert!(
            !self.minters.contains(account_id),
            "account_id exists minters list"
        );
    ...
    fn internal_delete_minter(&mut self, account_id: &AccountId) {
        assert!(
            self.minters.contains(account_id),
            "account_id not exists minters list"
        );
...
impl AnyTokenTrait for AnyToken {
    ...
    fn swap_in(&mut self, receiver_id: AccountId, amount: U128) -> Promise {
        ...
        match &self.underlying {
            Some(underlying) => ext_fungible_token::ft_balance_of(
                env::current_account_id().try_into().unwrap(),
                &underlying,
                NOT_DEPOSIT,
                BASE_GAS,
            )
            .then(ext_self::swap_in_callback(
